...
def test_record_entry_rejects_non_decimal_int_amount(acc_eur: Account):
    with pytest.raises(TypeError) as exc_info:
        acc_eur.record_entry(
            50,  # type: ignore[arg-type]  # int instead of Decimal
            JAN_01_2025,
            "GROCERIES",
            "EXPENSE",
        )
...
def test_record_entry_rejects_float_amount(acc_eur: Account):
    with pytest.raises(TypeError) as exc_info:
        acc_eur.record_entry(
            50.5,  # type: ignore[arg-type]  # float instead of Decimal
            JAN_01_2025,
            "GROCERIES",
            "EXPENSE",
        )
...
def test_record_entry_rejects_string_amount(acc_eur: Account):
    with pytest.raises(TypeError) as exc_info:
        acc_eur.record_entry(
            "50",  # type: ignore[arg-type]  # string instead of Decimal
            JAN_01_2025,
            "GROCERIES",
            "EXPENSE",
        )
