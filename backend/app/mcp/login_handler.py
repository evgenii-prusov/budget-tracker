...
async def _post_login(request: Request) -> HTMLResponse | RedirectResponse:
    ...
    if client is None:
        return _render_form(**form_kwargs, error="Unknown client.")

    registered_uris = [str(u) for u in (client.redirect_uris or [])]
    if redirect_uri not in registered_uris:
        return _render_form(**form_kwargs, error="Invalid redirect URI.")

    # Verify password via provider's public method
    if not provider.verify_password(password):
        return _render_form(**form_kwargs, error="Invalid password. Please try again.")
    ...
    # Redirect back to the client (safely merge query params)
    parts = urlsplit(redirect_uri)
    existing_params = parse_qs(parts.query, keep_blank_values=True)
    existing_params["code"] = [code]
    if state:
        existing_params["state"] = [state]
    new_query = urlencode(existing_params, doseq=True)
    redirect_url = urlunsplit((parts.scheme, parts.netloc, parts.path, new_query, parts.fragment))
